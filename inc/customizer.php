...
function academia_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'theme_bg_color',
				array(
					'label'		=> __( 'Color de fondo del header and footer', 'academia'),
					'section'	=> 'colors',
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'interactive_color', array(
				'label'		=> __( 'Color de interacción (links, etc)', 'academia' ),
				'section'	=> 'colors',
	...
	$wp_customize->add_section( 'theme_options',
		array(
			'title'			=> __( 'Opciones de la plantilla', 'academia' ),
			'priority'		=> 95,
			'capability'	=> 'edit_theme_options',
			'description'	=> __( 'Cambie cuánto de una entrada se muestra en el index y en la páginas de archivos.', 'academia' )
		)
	);
	...
	$wp_customize->add_setting(	'length_setting',
		array(
			'default'			=> 'excerpt',
			'type'				=> 'theme_mod',
			'sanitize_callback' => 'academia_sanitize_length', // Sanitization function appears further down
			'transport'			=> 'postMessage'
		)
	);

	// Add the controls
	$wp_customize->add_control(	'academia_length_control',
		array(
			'type'		=> 'radio',
			'label'		=> __( 'Mostrar en index/archivos', 'academia' ),
			'section'	=> 'theme_options',
			'choices'	=> array(
				'excerpt'		=> __( 'Resumen (default)', 'academia' ),
				'full-content'	=> __( 'Contenido completo', 'academia' )
			),
 ...
 */
function academia_customize_preview_js() {
	wp_enqueue_script( 'academia_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'academia_customize_preview_js' );
...
function academia_sanitize_length( $value ) {
...
if ( ! function_exists( 'academia_header_style' ) ) :
/**
 * Styles the header image and text displayed on the blog.
 *
 * @see academia_custom_header_setup().
 */
function academia_header_style() {
