...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="recurso__content">
			...
			<div class="recurso-cats">
				<!-- Ejes temáticos -->
				    <?php echo get_the_term_list(
				            $post->ID, 'temas', __('<span>', 'academia'), ', ', '</span>' ); ?>
				<!-- coberturas -->
				    <?php echo get_the_term_list(
				            $post->ID, 'cobertura', __('<span>', 'academia'), ', ', '</span>' ); ?>
				<!-- tipos de recurso -->
				    <?php echo get_the_term_list(
				        $post->ID, 'tipo_recurso', __('<span> ', 'academia'), ', ', '</span>' ); ?>
				<!-- Formato -->
				    <?php echo get_the_term_list(
				        $post->ID, 'tipo_medio', __('<span> ', 'academia'), ', ', '</span>' ); ?>
		...
		$read_more_link = sprintf(
			...
			wp_kses( __( 'Ver recurso %s', 'academia' ), array( 'span' => array( 'class' => array() ) ) ),
			the_title( '<span class="screen-reader-text">"', '"</span>', false )
		);
		?>

		<a href="<?php echo esc_url( get_permalink() ) ?>" class="read-more" rel="bookmark">
			<?php echo $read_more_link . academia_get_svg( array('icon' => 'arrow-right', 'fallback' => true ) ); ?>
		</a>
</article><!-- #post-## -->
